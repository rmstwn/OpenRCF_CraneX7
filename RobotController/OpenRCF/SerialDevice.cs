...
namespace OpenRCF
{
    public static class SerialDevice
    {
        ...
        public class Dynamixel
        {
            ...
            //nishi start
            public uint rad2DyAngle4servo(float rad)
            {
                uint result = new uint();
                result = (uint)((2048 + rad * 2048 / Math.PI) % 4096);
                //OpenRCFは0 degから角度が始まり、Dynamixelは180 degからはじまるように設計されているから

                return result;
            }

            public int deg2DyAngle(int deg)
            {
                int result = new int();
                result = (int)((deg * 2048 / 180) % 4096);

                return result;
            }
            public uint rad2DyAngle(float rad)
            {
                uint result = new uint();
                result = (uint)(rad * 2048 / Math.PI);
                //計算用
            ...
            public float DyAngle2rad4servo(int DyAngle)
            {
                float result = new float();
                result = (float)((-2048 + DyAngle) * Math.PI / 2048);
                //openRCFは0開始、Dynamixelは180 deg開始だから
